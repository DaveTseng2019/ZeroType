...
### 🧠 AI 驅動的語音辨識
- 支援 **OpenAI**、**Google Gemini** 與 **OpenRouter**（一把 Key 通吃多家模型）三種雲端語音辨識後端
- 支援 **本機** 後端：辨識全程在自己的 GPU 上跑，不需要 API Key，也不會產生費用。使用前要先啟動 LocalSTT 的端點；字典詞彙以熱詞形式送出，不進提示詞
...
### 📌 常用詞彙選擇器
- 同一句話不必再講第二次：按快捷鍵（預設 `Alt + C`）叫出浮窗，挑一句直接貼到游標所在的地方，走的是跟語音辨識結果一樣的貼上路徑
- 浮窗開在插入點旁邊。EmEditor、記事本、Office 這類用系統插入點的程式抓得到位置；Chrome、VS Code 自己畫游標，抓不到就退回螢幕上方三分之一置中
- 打字即時過濾（中文輸入法可用，候選字視窗會跟著關鍵字走）、`↑↓` 選、`Enter` 貼上、`Esc` 或點到別處取消
- 用鍵盤移動時滑鼠游標自動隱藏，反白不會被停在清單上的游標搶走
- 拖曳最上面的關鍵字列或最下面的提示列，可以把浮窗搬到不擋事的地方
- 詞彙來自歷史記錄的書籤按鈕，或直接編輯 `phrases.json`
- 浮窗出現時播提示音（設定 → 音效可挑選，也可單獨關掉這一個）

### ⚙️ 設定頁面
- 深色 / 淺色模式切換
- 開機自動啟動；開啟後可再勾選「啟動時縮小至系統匣」，開機不跳視窗
- 快捷鍵自訂（支援任意組合鍵）：一般錄音、精簡模式與常用詞彙各一組，設成同一組會被擋下
- 錄音開始 / 結束、失敗與常用詞彙提示音都可挑選系統內建音效
- 麥克風權限與輔助使用權限狀態即時顯示
...
### [v1.0.28] 當前版本
- **本機辨識的端點由 ZeroType 自己顧** 🧠 — 以前要先自己把 LocalSTT 的服務跑起來，ZeroType 才連得上；忘了開就是一句話丟出去沒有反應。現在服務商選「本機」就會自動啟動端點，關掉 ZeroType 時一併收掉。面板上看得到狀態、有啟動／停止鍵，也有端點記錄可以展開；模型常駐的約 1.8 GB 顯示記憶體，暫時不用時按「停止」就收得回來。
- **不用填任何路徑** 📂 — 端點程式放在 `C:\Learning\LocalSTT`、使用者資料夾底下或 ZeroType 執行檔旁邊的 `LocalSTT`，都會自動找到。這台電腦上還沒裝的時候，面板不會叫你去填路徑，而是給一段寫好的安裝說明，按一下複製起來貼給 AI 助理或請人代勞就好。
- **端點在背景跑，不會再冒出一個關不得的黑視窗** 🪟 — 之前啟動後會多一個空白的主控台視窗，關掉它等於把辨識服務殺掉。現在建立行程時就不配視窗，畫面上乾乾淨淨。想看即時輸出的人，設定裡有「啟動時顯示主控台視窗」；打開時視窗裡才真的有字，不再是一片空白。
- **要換別的本機模型也改得動** 🔧 — 「進階設定 → 自建模型接口」填的網址現在同時決定健康檢查與關閉端點要打哪裡，不會再固定認 8123。「啟動設定（進階）」可以指定別的端點程式、參數與記錄檔，留空就是自動偵測。平常不用動這一區。

### [v1.0.27]
- **常用詞彙可以排順序、就地修改** 📌 — 拖曳左側把手調整順序，順序會存進 `phrases.json`，`Alt + C` 選擇器裡的順序跟著一樣。點一列後按 `F2`（或右側鉛筆圖示）直接改，`Enter` 存、`Esc` 取消；不在編輯狀態時用 `↑/↓` 移動選取列。詞彙前後的空白原樣保留。

### [v1.0.26]
- **常用詞彙變成「挑一句直接輸入」的工具** 📌 — 過去常用詞彙只能在 ZeroType 視窗裡按複製，要用還得自己切回去貼一次，同一句話重講一遍反而比較快。現在按 `Alt + C`，游標旁邊就浮出一個小視窗：打字即時過濾（中文輸入法可用）、`↑↓` 選、`Enter` 直接貼進你正在打字的地方，走的是跟語音辨識結果一模一樣的貼上路徑，貼完照樣把剪貼簿還原。用鍵盤移動時滑鼠游標自動隱藏，反白不會被停在清單上的游標搶走；拖曳最上或最下那一列可以把浮窗搬到不擋事的地方。
- **浮窗點到別處不會消失** 🟠 — 挑詞的時候常常要回頭看原本的畫面。點到別的視窗浮窗會留著，只把外框從橘轉灰（灰＝打字不會進來）；再按一次 `Alt + C` 就把焦點要回來，清單和打過的關鍵字都還在，第三次才收起。原本要貼上的視窗被關掉時浮窗會自己收掉——沒地方可貼了，留著只是擋路。
- **設定頁的開關縮到跟文字相稱** 🎛️ — 原本的 Switch 比旁邊的標題字大一截。五顆全部縮小，數值跟紀錄頁的「偵錯模式」一致。深色模式那顆圖示按鈕比開關窄 8px，靠右對齊時中線會歪掉，補了邊距把它推回同一條垂直線。
- **新增常用詞彙提示音** 🔔 — 浮窗開在插入點旁邊，眼睛不一定正好在那裡。設定 → 音效多了一顆「常用詞彙音效」，另有獨立開關可以只關掉這一個而不動其他提示音。
- **紀錄文字精簡** 📝 — `貼上目標：` 改成 `貼上：`、`記到的焦點=` 改成 `焦點=`。中文寫動作與重點，英文欄位（`hwnd= pid= cls= focus=`）維持原樣——那些是跟 Windows 問來的原始資料，翻譯了反而看不出哪些值可以拿去查。

### [v1.0.25]
- **新增 API 延遲紀錄** ⏱️ — 辨識偶爾要等很久，但事後沒有任何地方查得到當時花了多少秒。現在每次辨識都把兩段 API 各自的耗時逐行寫進 `latency.jsonl`（時間、服務商、模型、毫秒、音檔大小）：**音訊段**與**字典校正段**分開記，才看得出是哪一段在拖；**失敗也記**，因為「延遲很久」最後往往就是逾時，只記成功的會剛好漏掉最想看的那幾筆。刻意不併進歷史記錄——那份有保留天數會過期，但拿來比較服務商就是要長期累積的資料。
- **偵錯模式開關搬到「紀錄」頁** 🐞 — 原本在設定 → 一般設定最下方，但它作用的地方是紀錄頁。現在開關就在紀錄頁標題列上，開關旁邊那行字同時告訴你目前是只留在記憶體還是有寫進 `debug.log`。
- **「開啟目錄」改名「工作目錄」，並改開上層資料夾** 📂 — 過去這顆按鈕直接進到 `history_audio` 裡面，只看得到音檔。現在開在它的上層，`history.json`、`latency.jsonl`、`debug.log` 和音檔資料夾一眼全在。
- **移除紀錄頁的「紀錄檔位置」按鈕** 🗑️ — `debug.log` 本來就跟其他檔案在同一個資料夾，跟歷史頁的「工作目錄」完全重疊，留兩顆只是多一個要維護的地方。

### [v1.0.24]
- **新增 Gemini 3.7 Flash** ⚡ — OpenRouter 的模型清單多了 `google/gemini-3.7-flash`，音訊每百萬 token $0.375、輸出 $1.875，是目前清單裡最便宜的 Gemini。
- **費用改成 OpenRouter 回報的實際扣款金額** 💰 — 過去是拿程式裡寫死的價目表估算，模型一改價就失準：Gemini 3.6 Flash 在 OpenRouter 無聲降價一半，「紀錄」頁算出來的費用因此高了一倍。現在每次辨識直接向 API 要這次實際扣了多少錢，連快取折扣都算得進去，價格再怎麼變都不會失真。選單上的費率同樣是即時抓的，抓不到才退回上一次的快取。（OpenAI 與 Gemini 原生 API 不回報金額，那兩家仍是估算值。）
- **模型清單不再列出批次專用的版本** 🚫 — 清單直接抓 OpenRouter 的即時資料，裡面混著結尾是 `:batch` 的變體，價格正好是一半、看起來最划算，但它們只能走批次 API，選下去每次辨識都會失敗（`DioException ... 404`）。現在一律過濾掉。

### [v1.0.23]
- **提示音回到按下熱鍵的那一刻** 🔔 — 沒開「等待麥克風就緒」時，提示音其實是在等你先開口講到夠大聲才響：安靜開場就變成慢半拍，一直沒出聲甚至整場都不會響，但錄音從按下去的那一刻就已經在收了。現在按下熱鍵、麥克風一送資料就出聲，不必先講話。（有開等待的行為不變，提示音照舊在麥克風真的就緒時才響。）

### [v1.0.22]
- **錄音提示改成獨立浮窗** 🎈 — 過去的提示藥丸畫在程式視窗內，視窗被蓋住或縮到系統匣時等於不存在，你只能靠系統匣圖示猜自己有沒有在錄。現在它是一個**永遠置頂、不搶焦點**的小視窗，固定浮在工作列正上方、螢幕水平置中，不管當下在哪個程式前面都看得到。點右邊的 ✕ 取消，Esc 照舊。（多螢幕時只出現在主螢幕的工作列上方。）
- **提示會告訴你按到哪一顆熱鍵** ⌨️ — 兩顆熱鍵的行為差很多：全局模式要再按一次才停，精簡模式講完自動停、還會補 Enter 送出。提示現在直接寫「錄音中（全局）」或「錄音中（精簡）」，系統匣圖示的 tooltip 也一樣，滑過去就知道。按錯不必等它自己收掉才發現。
- **提示配色重調** 🎨 — 原本整條藥丸都是紫色，在近黑底上對比只有 3.94:1，3px 寬的波形條幾乎看不見。現在文字與 ✕ 一律白色（15.6:1），階段改由圓點與波形條的顏色帶：準備中橘、錄音中綠、擷取中橘、辨識中淺藍。
...
- **沒錄到聲音時告訴你量到多少** 📉 — 「只錄到底噪」的訊息從「動態範圍低於 4.0」改成報**這次實測的值**（例如「動態範圍 2.3」）。被誤擋時你才看得出離門檻多遠。
- **保留天數搬到歷史頁** 🗂️ — 歷史記錄的保留天數不必再去設定頁找，直接在歷史頁標題下方調整（7／14／30 天，最多 30 天），旁邊寫明到期的記錄與音檔會在**下次啟動時**自動刪除。
